...
- **纯命令行交互（2026-08-18 修订）**：映射选择为编号数字菜单 + `Read-Host`，无 GUI 分支。初版在 Out-GridView 可用且交互会话时优先 GUI、否则回退数字菜单；为保持交互路径唯一、行为一致而移除 GUI 分支。未命中映射时非交互会话会因 `Read-Host` 不可用而失败，属预期——映射必须在交互会话中建立。
